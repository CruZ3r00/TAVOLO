...
- **`strapi/`** — Strapi v5 CMS + admin. REST API on `http://localhost:1337`. Hosts auth + internal services. → `strapi/CLAUDE.md`
- **`vuejs/frontend/`** — Vue 3 + Vite SPA. Bootstrap 5, Vuex, Vue Router with role-based guards. Single owner + multi-staff (gestione, cameriere, cucina, bar, pizzeria, cucina_sg). → `vuejs/frontend/CLAUDE.md`
- **`ocr-service/`** — Python 3.10+ FastAPI on `127.0.0.1:8001`. PDF/image → JSON pipeline (PyMuPDF + OpenCV + PaddleOCR + local Ollama LLM). Called only by Strapi. → `ocr-service/CLAUDE.md`
- **`pos-rt-service/`** — Node.js daemon (Win/Linux/macOS) + Capacitor mobile app. Drives POS terminals, printers, fiscal registers. → `pos-rt-service/CLAUDE.md`
...
- **Strapi content types, REST API, payment strategy, migrations** → `strapi/CLAUDE.md`
- **Frontend pages/components/router/store/utils** → `vuejs/frontend/CLAUDE.md`
- **OCR pipeline, env, startup** → `ocr-service/CLAUDE.md`
- **POS-RT daemon, drivers, mobile app** → `pos-rt-service/CLAUDE.md`
...
- **OCR:** see `ocr-service/CLAUDE.md`.
- **POS-RT:** see `pos-rt-service/CLAUDE.md`.
